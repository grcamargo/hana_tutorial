    ...
    _fetchAccessToken() {
        return new Promise((resolve, reject) =>
            this._validateAccessToken() ?
                ...
                request.post(
                    {
                        url: this._credentials.uaa.url + '/oauth/token',
                        form: { response_type: 'token', grant_type: 'client_credentials' },
                        auth: {
                            username: this._credentials.uaa.clientid,
                            password: this._credentials.uaa.clientsecret,
                            sendImmediately: true
                        }
                    },
                    (error, response, body) => {
                        if (error || response.statusCode !== 200) {
                            reject(error || new Error('Error fetching access token from ' + this._credentials.uaa.url));
                        } else {
